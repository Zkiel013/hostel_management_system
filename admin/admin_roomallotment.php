...
<body>
            ...
            <div class="roomDetails">
                ...
                <div id="roommate_details">
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['select_room'])) {
                        ...
                        //no of occupants (0/1)
                        $query = "SELECT `status`, `occupant1_roll_no`, `occupant2_roll_no` FROM $table_name WHERE `room_no` = $room_number";
                        $result = mysqli_query($conn, $query);

                        if (mysqli_num_rows($result) > 0) {
                            // If rows were returned, fetch the data
                            $row = mysqli_fetch_assoc($result);
                            $no_of_occupant = $row['status']; // Set no of occupants based on status
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['rollno']) && isset($_POST['add_student'])) {
        ...
        if ($rollno_alloted > 0) {
            ...
            $q_no_of_occupant = "SELECT `status` FROM $table_name WHERE `room_no` = $room_number";
            ...
            if ($no_of_occupant > 0) { // 1. add new record
                ...
                $query = "INSERT INTO $table_name (`room_no`, `occupant1_roll_no`, `status`)
